'''qestion_classifier.py: 问句分类器'''

import os
import ahocorasick
from symptom_matcher import SymptomMatcher
from spacy_matcher import SpacyMatcher
...
class QuestionClassifier:
    def __init__(self):
        ...
        # 问句疑问词
        self.other_qwds = ['咋样', '你', '我', '它’,‘她', '概率', '作', '来', '成因', '吃', '周期', '躲避', '逃开',
                           '菜谱', '原因', '并发症', '表现', '属于', '喝', '治愈', '口服液', '感染', '并发', '现象',
                           '免得', '方式', '治', '药', '抵制', '药品', '用处', '医治', '有', '导致', '发生', '用途',
                           '造成', '防范', '测出', '大概率', '表征', '忌口', '食物', '治疗', '症状', '饮食', '几成',
                           '用药', '疗法', '可以', '咋治', '可能性', '时间', '人群', '科', '症候', '会', '上', '伴随',
                           '保健品', '胶囊', '伙食', '治啥', '食', '人', '用来', '项目', '共现', '医', '几率', '抵御',
                           '需要', '小时', '试出', '食谱', '大么', '办', '防止', '检查', '比例', '逃避', '怎么办', '菜',
                           '饮用', '食用', '躲掉', '查出', '绕开', '何益处', '避开', '可治', '躲开', '科室', '炎片',
                           '出现', '补品', '要', '预防', '避掉', '他', '治疗']
    ...
    def classify(self, question):
        ...
        if not medical_dict:
            # 处理句子,利用spacy提取关键词
            subject_entities = self.nlp.match(question)

            if subject_entities:
                for entity in subject_entities:
                    if entity in self.other_qwds:
                        question = str(self.context['previous_question']) + ' ' + question
                        medical_dict = self.check_medical(question)
                    else:
                        # 使用SimBERT进行模糊识别
                        best_match = self.matcher.find_best_match(question)
                        medical_dict = {best_match: self.wdtype_dict.get(best_match, [])}
            else:
                question = str(self.context['previous_question']) + ' ' + question
                medical_dict = self.check_medical(question)
